...
import random
import time
with open("Q.json", "r", encoding='utf-8') as read_file:
    Q = json.load(read_file)
...
class Breakout(Game):
    def __init__(self):
        ...
        self.TextQuest = TextsQuest(Q)
        self.time_spawn_text = []
    ...
    def create_labels(self):
        ...
        self.iscore_label = TextObject(100,
                                      c.status_offset_y,
                                      lambda: f'INT SCORE: {self.intScores}',
                                      "green",
                                      c.font_name,
                                      c.font_size)
        self.objects.append(self.iscore_label)
        self.timeQ_label = TextObject(635,
                                      c.status_offset_y,
                                      lambda: f'TIME: {self.time_q}',
                                      "deeppink",
                                      c.font_name,
                                      c.font_size)
        self.objects.append(self.timeQ_label)
    ...
    def update(self):
        ...
        if len(self.time_spawn_text) == 0:
            self.time_spawn_text.append(time.time() + random.randint(4,8)) # randint показывает через сколько запустить вопрос
        else:
            if self.time_spawn_text[0] <=time.time(): # Наступил момент запуска вопроса
                print("Create text")
                if len(self.TextQuest.texts) == 0:
                    print(len(Q))
                    self.TextQuest.create_text(random.randint(0,len(Q)-1))# Рандомный вопрос
                    self.choose = 1
                    self.last_choose = 1
                    self.is_answere_true = False
                    self.is_answere_False = False
                    self.time_startQ = time.time()
                    self.time_q = 0
                else:

                    if self.time_startQ+6<time.time(): # После 6 сек вопрос исчезает
                        self.TextQuest.delete_text()
                        self.time_spawn_text.clear()
                        self.time_q = 0
                        if self.lives>0:
                          self.lives -=1
                    ...
                    # Если создано
                        if self.is_answere_true:
                            self.TextQuest.delete_text()
                            self.time_spawn_text.clear()
                            self.intScores += 1
                            # dtc = random.randint(0,1)
                            self.lives += 1# 50% на добавление жизни
                            #Удаляет надпись таймер
                            # Доабвляет очки I и отображает
                            
                            self.time_q = 0


                        else:
                            if self.is_answere_False:
                                #Неверный ответ
                                self.TextQuest.delete_text()
                                self.time_spawn_text.clear()
                                self.time_q = 0
                                ...
                                self.time_q = int(7 - (time.time()-self.time_startQ))

                                self.TextQuest.spawn(self.surface,self.ball.center)

                                font = pygame.font.SysFont('Verdana', 20)# вынести в конфиг size

                                self.TextQuest.texts[self.last_choose][0] = font.render(self.TextQuest.texts[self.last_choose][1], True, self.TextQuest.texts[self.last_choose][2])# Истина цвет
                                self.TextQuest.texts[self.choose][0] = font.render(self.TextQuest.texts[self.choose][1], True, "white")# Белый цвет


                                self.last_choose= self.choose



        # print("update ",self.ball.center)
        self.handle_ball_collisions()
        super().update()

        if self.game_over:
            self.show_message('GAME OVER!', centralized=True,end=1 )

    def show_message(self, text, color=colors.WHITE, font_name='Arial', font_size=20, centralized=False,end=0):
        message = TextObject(c.screen_width // 2, c.screen_height // 2, lambda: text, color, font_name, font_size)
        color = "green"
        messageSc = TextObject(c.screen_width // 2-50, c.screen_height // 2+25, lambda: f'SCORE: {self.score}', color, font_name, font_size)
        messageScInt = TextObject(c.screen_width // 2-50, c.screen_height // 2+25+25, lambda: f'INT Score: {self.intScores}', color, font_name, font_size)
        messageScAll = TextObject(c.screen_width // 2-50, c.screen_height // 2+25+25+25, lambda: f'Всего: {self.score + self.intScores}', color, font_name, font_size)

        self.draw()
        message.draw(self.surface, centralized)
        if end == 1:
            centralized = False
            messageSc.draw(self.surface, centralized)
            messageScInt.draw(self.surface, centralized)
            messageScAll.draw(self.surface, centralized)
